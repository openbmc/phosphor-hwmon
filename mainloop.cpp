              ...
              bool isOCC = false)
{
    ...
    if (sensor::getAttributes(sensor.first, attrs))
    {
        iface->unit(sensor::getUnit(attrs));
...
void MainLoop::getObject(SensorSet::container_t::const_reference sensor)
{
    ...
    sensor::Attributes attrs;
    if (!sensor::getAttributes(sensor.first.first, attrs))
    {
        return;
    }
    ...
    objectPath.append(1, '/');
    objectPath.append(sensor::getNamespace(attrs));
    objectPath.append(1, '/');
