                                              ...
                                              TimedoutMap& timedoutMap)
{
    ...
    if (statusIface->functional())
    {
...
#endif
        {
            ...
            {
                val = asyncRead(_sensor, _ioAccess, std::stoi(asyncReadTimeout),
...
SensorValueType asyncRead(const SensorSet::key_type& sensorSetKey,
                          ...
                          const std::string& sensor, const size_t retries,
                          const std::chrono::milliseconds delay)
{
    // Default async read timeout
    std::chrono::milliseconds asyncReadTimeout{asyncTimeout};
    ...
    switch (status)
    {
        ...
        default:
            ...
            // stack. The destructor will otherwise block until the read completes
            // due to the limitation of std::async.
