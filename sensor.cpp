    ...
    if (statusIface->functional())
    {
...
#endif
        {
            ...
            if (!asyncRead.empty())
            {
                ...
                auto asyncIter = asyncMap.find(_sensor);
                if (asyncIter == asyncMap.end())
                {
                    auto emplacePair = asyncMap.emplace(
                        _sensor,
                        std::async(
                            std::launch::async,
                ...
                std::future_status status = async.wait_for(asyncReadTimeout);
                switch (status)
                {
                    // Read has finished
                    case std::future_status::ready:
                        val = async.get();
                        asyncMap.erase(_sensor);
                        if (valueIsValid)
                        {
                            break;
                        }
                    ...
                    default:
                        throw std::runtime_error("Async sensor read timed out");
                }
            }
            else
            {
        ...
        catch (const std::system_error& e)
        {
            ...
            // Erase from the map if the first read actually threw an exception
            if (std::string(e.what()) != "Async sensor read timed out")
            {
                asyncMap.erase(_sensor);
            }
        }
