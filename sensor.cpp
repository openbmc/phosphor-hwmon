...
// helper class to set the thershold interface only when it's available
template <typename T>
void setThreshold(T& iface, int64_t value, double)
{
}
template <typename T>
void setThreshold(T& iface, int64_t value, int64_t)
{
    iface->scale(value);
}
                                              ...
                                              ObjectInfo& info)
{
    ...
    if (hwmon::getAttributes(sensor.first, attrs))
    {
        ...
        setThreshold(iface, hwmon::getScale(attrs), val);
