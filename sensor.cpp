...
{
    auto chip = env::getEnv("GPIOCHIP", _sensor);
    auto access = env::getEnv("GPIO", _sensor);
    ...
    auto gain = env::getEnv("GAIN", _sensor);
    if (!gain.empty())
    {
        _sensorAdjusts.gain = std::stod(gain);
    }

    auto offset = env::getEnv("OFFSET", _sensor);
    if (!offset.empty())
    {
        _sensorAdjusts.offset = std::stoi(offset);
    }
    auto senRmRCs = env::getEnv("REMOVERCS", _sensor);
