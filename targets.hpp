                             ...
                             ObjectInfo& info)
{
    ...
    {
        targetName = pwm;
        targetId = getEnv("PWM_TARGET", sensor);
        if (targetId.empty())
        {
            // If no env, fall back to use the sensor id as target id
            targetId = sensor.second;
        }
        entry = empty;
    }
    else
    {
        targetName = sensor.first;
        targetId = sensor.second;
        entry = hwmon::entry::target;
    }
