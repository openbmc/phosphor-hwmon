...
ReadCache::~ReadCache()
{
    ...
    for (auto const& entry : _sensorMap)
    {
        std::int32_t fd = entry.second->_fd;
        if (fd != -1 && close(fd) == -1)
        {
            log<level::ERR>(
                fmt::format("io_uring FAIL: close() of {} failed ({})",
                            entry.first, std::strerror(errno))
                    .c_str());
        }
    }
}
                                       ...
                                       const std::string& sensor)
{
    ...
    if (_sensorMap.count(path) == 0)
    {
        ...
        resultPtr->_handler = std::make_unique<CacheCQEHandler>(path, this);
...
std::int32_t ReadCache::processReadRequest(const std::string& path)
{
    ...
    io_uring_prep_read(&sqe, ptr->_fd, ptr->_readBuffer.data(), MAX_READ_SIZE,
                       0);
