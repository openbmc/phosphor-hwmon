...
ReadCache::ReadCache(const std::uint32_t size, const std::string& path,
    ...
    _hwmonPath(path),
    _retries(retries), _ring(std::make_unique<stdplus::IoUring>(size))
{
                                       ...
                                       const std::string& sensor)
{
    ...
    if (_sensorMap.count(pathKey) == 0)
    {
        std::shared_ptr<ReadResult> resultPtr = std::make_shared<ReadResult>(
            std::make_unique<CacheCQEHandler>(pathKey, this), 0, 0, 0,
            _retries);
    ...
    if (std::count(retryableErrors.begin(), retryableErrors.end(), rc) == 0 ||
...
void ReadCache::submitReadRequest(const std::string& path)
{
    ...
    io_uring_prep_read(&sqe, fd, &(ptr->_value), sizeof(std::int64_t), 0);
...
ReadCache::ReadResult::ReadResult(std::unique_ptr<CacheCQEHandler> handler,
                                  std::int64_t value, int fd, int rc,
