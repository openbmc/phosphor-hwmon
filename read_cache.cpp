                                       ...
                                       const std::string& sensor)
{
    ...
    if (_sensorMap.count(path) == 0)
    {
        ...
        try
        {
            ...
            initialValue = _ioAccess->read(type, id, sensor, hwmonio::retries,
                                           hwmonio::delay);
        }
        catch (const std::exception& e)
        {
            log<level::ERR>(
                fmt::format("io_uring FAIL: sync read {} failed ({})", path,
                            e.what())
                    .c_str());
            throw;
        }
    ...
    /*
     * If the directory or device disappeared then this application should
     * gracefully exit. There are race conditions between the unloading of a
     * hwmon driver and the stopping of this service by systemd. To prevent
     * this application from falsely failing in these scenarios, it will simply
     * exit if the directory or file can not be found. It is up to the user(s)
     * of this provided hwmon object to log the appropriate errors if the object
     * disappears when it should not.
     */
    if (rc == ENOENT || rc == ENODEV)
    {
        log<level::INFO>(
            fmt::format("io_uring: exiting with ({})", std::strerror(rc))
                .c_str());
        exit(0);
    }
...
void ReadCache::processReadRequest(const std::string& path)
{
